...
void inline Process_Keymaster_Version(TWPartition *ven, bool Display_Error) {
	...
	TWFunc::Get_Service_From_Manifest("/vendor", "android.hardware.keymaster", service);
	if (strstr(service.c_str(), "4")) {
		service = "4.x";
	}
	...
	if (service.empty()) {
		...
		service = android::base::GetProperty(TW_KEYMASTER_VERSION_PROP, service);
		if (service.empty()) {
