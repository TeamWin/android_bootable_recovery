...
#ifdef TW_INCLUDE_FBE
  //  getprop: [twrp.user.999.decrypt]: [0]
  //  when twrp.user.*.decrypt=0 need decrypt the user
  char decrypted_state[PROPERTY_VALUE_MAX];
  string cmd = "getprop | grep twrp.user. | cut -d']' -f1 | cut -d'.' -f3";  // get users ID
  string res_str = "";
  TWFunc::Exec_Cmd(cmd, res_str);
  LOGINFO("DEBUG: Users being parsed:\n %s", res_str.c_str());
  if (res_str != "") {
    std::vector<std::string> prop_user_ids = TWFunc::Split_String(res_str, "\n");
    for (size_t i = 0; i < prop_user_ids.size(); ++i) {
      int prop_user_id = atoi(prop_user_ids[i].c_str());
      if (prop_user_id < 0 || prop_user_id > 9999) {
        LOGINFO("Incorrect user id %d\n", prop_user_id);
        continue;
      }
      struct users_struct user;
      user.userName = prop_user_ids[i];
      user.userId = prop_user_ids[i];

      string filename;
      user.type = Get_Password_Type(prop_user_id, filename);

      user.isDecrypted = false;
      string user_prop_decrypted = "twrp.user." + prop_user_ids[i] + ".decrypt";
	  property_get(user_prop_decrypted.c_str(), decrypted_state, "");
	  if (strcmp(decrypted_state, "1") == 0)
...
void TWPartitionManager::Mark_User_Decrypted(int userID) {
  ...
  for (iter = Users_List.begin(); iter != Users_List.end(); iter++) {
    if (atoi((*iter).userId.c_str()) == userID) {
	  ...
	  LOGINFO("DEBUG: Setting user %d as decrypted\n", userID);
...
int TWPartitionManager::Decrypt_Device(string Password, int user_id) {
    ...
    std::vector<users_struct>::iterator iter;
    for (iter = Users_List.begin(); iter != Users_List.end(); iter++) {
		LOGINFO("DEBUG: Iterating through user list, on user %s\n", (*iter).userId.c_str());
		LOGINFO("DEBUG: Wants user %d\n", user_id);
		if ((*iter).isDecrypted) {
			LOGINFO("DEBUG: User %s is marked in list as decrypted\n", (*iter).userId.c_str());
		}
		else {
			LOGINFO("DEBUG: User %s is not marked in list as decrypted\n", (*iter).userId.c_str());
		}
    ...
    if (!user_need_decrypt) {
      LOGINFO("User %d has no decryption required\n", user_id);
      return 0;
    }
    ...
    gui_msg(Msg("decrypting_user_fbe=Decrypting FBE for user {1}...")(user_id));
    if (Decrypt_User(user_id, Password)) {
      ...
      if (user_id == 0) {
        // when decrypted user 0, try to decrypt all other users. such as when using default
        // password
        std::vector<users_struct>::iterator iter;
        for (iter = Users_List.begin(); iter != Users_List.end(); iter++) {
          if ((*iter).userId == "0" || (*iter).isDecrypted) continue;

          int tmp_user_id = atoi((*iter).userId.c_str());
          // LOGINFO("Decrypting FBE for user %i\n", tmp_user_id);
          gui_msg(Msg("decrypting_user_fbe=Decrypting FBE for user {1}...")(tmp_user_id));
          if (Decrypt_User(tmp_user_id, Password) ||
              (Password != "!" && Decrypt_User(tmp_user_id, "!"))) {  // "!" means default password
            // LOGINFO("User %i Decrypted Successfully!\n", tmp_user_id);
            gui_msg(Msg("decrypt_user_success_fbe=User {1} Decrypted Successfully")(tmp_user_id));
            Mark_User_Decrypted(tmp_user_id);
          } else {
            LOGINFO("User %i is unable to decrypt!\n", tmp_user_id);
          }
        }
        Post_Decrypt("");
      }
