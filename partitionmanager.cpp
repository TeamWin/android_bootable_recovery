...
void inline RESET(std::string prop, std::string def, TWPartition *ven, TWPartition *odm) {
	...
	if (odm) {
		...
		if (!odm_prop.empty()) {
			prop_on_odm = true;
			if (TWFunc::Property_Override(prop, odm_prop) == -2) {
	...
	if (ven) {
		...
		if (!vendor_prop.empty()) {
			prop_on_vendor = true;
			if (TWFunc::Property_Override(prop, vendor_prop) == -2) {
				LOGERR("Unable to override '%s' due to missing libresetprop\n", prop.c_str());
			} else {
				prop_value = android::base::GetProperty(prop, "");
	...
	if (!prop_on_odm && !prop_on_vendor && !def.empty()) {
		if (TWFunc::Property_Override(prop, def) == -2) {
			LOGERR("Unable to override '%s' due to missing libresetprop\n", prop.c_str());
		} else {
			prop_value = android::base::GetProperty(prop, "");
	...
	if (!prop_on_odm && !prop_on_vendor && !prop_value.empty() && def.empty()) {
		if(TWFunc::Delete_Property(prop) == -2) {
