...
int TWPartitionManager::Decrypt_Device(string Password, int user_id) {
	...
	if (DataManager::GetIntValue(TW_IS_FBE)) {
		...
		if (Decrypt_User(user_id, Password)) {
			...
			if (user_id == 0) {
				// when decrypted user 0, try to decrypt all other users. such as when using default password
				std::vector<users_struct>::iterator iter;
				for (iter = Users_List.begin(); iter != Users_List.end(); iter++) {
					...
					if (Decrypt_User(tmp_user_id, Password) || (Password != "!" && Decrypt_User(tmp_user_id, "!"))) { // "!" means default password
