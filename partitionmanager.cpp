...
clear:
	...
	if (!parse_userdata) {
		...
		if (TWFunc::Find_Fstab(Fstab_Filename)) {
			LOGINFO("Fstab: %s\n", Fstab_Filename.c_str());
			Fstab out;
			ReadFstabFromFile(Fstab_Filename, &out);
			for (auto& entry : out) {
				fstab.emplace_back(std::move(entry));
			}
			parse_userdata = true;
#define RESET(prop) \
	TWFunc::Property_Override(prop, TWFunc::Partition_Property_Get(prop, PartitionManager, "/odm", "build.prop")); \
	...
	LOGINFO("Settings '%s' to %s\n", prop, android::base::GetProperty(prop, "").c_str());

			RESET("ro.crypto.dm_default_key.options_format.version");
			RESET("ro.crypto.volume.metadata.method");
			RESET("ro.crypto.volume.options");
#undef RESET
			goto parse;
		} else {
...
#ifdef TW_INCLUDE_FBE_METADATA_DECRYPT
#ifdef USE_FSCRYPT
			if (android::vold::fscrypt_mount_metadata_encrypted(Decrypt_Data->Actual_Block_Device, Decrypt_Data->Mount_Point, false, false, Decrypt_Data->Current_File_System, &fstab)) {
