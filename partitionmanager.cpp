...
void inline RESET(std::string prop, std::string def, TWPartition *ven, TWPartition *odm) {
	if (odm) TWFunc::Property_Override(prop, TWFunc::Partition_Property_Get(prop, PartitionManager, "/odm/etc", "build.prop"));
	if (ven) TWFunc::Property_Override(prop, TWFunc::Partition_Property_Get(prop, PartitionManager, "/vendor", "build.prop"));
	if (android::base::GetProperty(prop, "") == "" && !def.empty()) TWFunc::Property_Override(prop, def);
