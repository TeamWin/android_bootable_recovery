...
bool TWPartition::Wipe(string New_File_System) {
	...
	if (Has_Data_Media && Current_File_System == New_File_System) {
		...
		if (Mount_Point == "/data" && TWFunc::get_cache_dir() == AB_CACHE_DIR) {
			...
			gui_msg(Msg(msg::kWarning, "fbe_wipe_msg=WARNING: {1} recreated. FBE device should be booted into Android and not Recovery to set initial FBE policy after wipe.")(TWFunc::get_cache_dir()));
		}
...
void TWPartition::Recreate_AB_Cache_Dir(const ext4_encryption_policy &policy) {
	struct passwd pd;
