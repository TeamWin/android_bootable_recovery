...
bool TWPartition::Wipe(string New_File_System) {
	...
	if (Mount_Point == "/data" && TWFunc::get_log_dir() == TWRP_AB_LOGS_DIR && Is_Decrypted) {
		TWFunc::Get_Encryption_Policy(policy, "/data/app");
	}
	...
	if (Has_Data_Media && Current_File_System == New_File_System) {
		...
		if (Mount_Point == "/data" && TWFunc::get_log_dir() == TWRP_AB_LOGS_DIR) {
			bool created = Recreate_TWRP_AB_Logs_Dir(policy);
			if (!created)
...
#ifndef TW_OEM_BUILD
		if (Is_FBE)
			gui_msg(Msg(msg::kWarning, "fbe_wipe_msg=WARNING: {1} wiped. FBE device should be booted into Android and not Recovery to set initial FBE policy after wipe.")(TWFunc::get_log_dir()));
		else
...
bool TWPartition::Recreate_TWRP_AB_Logs_Dir(const ext4_encryption_policy &policy) {
#ifdef TW_INCLUDE_FBE
	struct passwd pd;
	...
	if ((getpwnam_r("system", pwdptr, pwdBuf, sizeof(pwdBuf), &tempPd)) != 0) {
		...
		if ((getgrnam_r("cache", grpptr, grpBuf, sizeof(grpBuf), &tempGrp)) != 0) {
			...
			uid = pd.pw_uid;
			gid = grp.gr_gid;
			std::string abLogsRecoveryDir(TWRP_AB_LOGS_DIR);
			...
			LOGINFO("setting policy for %s: %s\n", policy_hex, abLogsRecoveryDir.c_str());
			if (sizeof(policy.master_key_descriptor) > 0) {
				if (!TWFunc::Set_Encryption_Policy(abLogsRecoveryDir, policy)) {
					return false;
				}
			} else {
