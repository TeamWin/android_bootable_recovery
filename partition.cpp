...
bool TWPartition::Wipe(string New_File_System) {
	...
	if (Mount_Point == "/data" && TWFunc::get_cache_dir() == TWRP_AB_LOGS_DIR && Is_Decrypted) {
	...
	if (Has_Data_Media && Current_File_System == New_File_System) {
		...
		if (Mount_Point == "/data" && TWFunc::get_cache_dir() == TWRP_AB_LOGS_DIR) {
...
bool TWPartition::Recreate_TWRP_AB_Logs_Dir(const ext4_encryption_policy &policy) {
	...
	if ((getpwnam_r("system", pwdptr, pwdBuf, sizeof(pwdBuf), &tempPd)) != 0) {
		...
		if ((getgrnam_r("cache", grpptr, grpBuf, sizeof(grpBuf), &tempGrp)) != 0) {
			...
			if (!TWFunc::Create_Dir_Recursive(TWRP_AB_LOGS_DIR, S_IRWXU | S_IRWXG | S_IWGRP | S_IXGRP, uid, gid)) {
				LOGERR("Unable to recreate %s\n", TWRP_AB_LOGS_DIR);
				return false;
			}
			if (setfilecon(TWRP_AB_LOGS_DIR, "u:object_r:cache_file:s0") != 0) {
				LOGERR("Unable to set contexts for %s\n", TWRP_AB_LOGS_DIR);
				return false;
			}
			char policy_hex[EXT4_KEY_DESCRIPTOR_SIZE_HEX];
			policy_to_hex(policy.master_key_descriptor, policy_hex);
			LOGINFO("setting policy for %s: %s\n", policy_hex, TWRP_AB_LOGS_DIR);
			if (sizeof(policy.master_key_descriptor) > 0) {
				if (!TWFunc::Set_Encryption_Policy(TWRP_AB_LOGS_DIR, policy)) {
					LOGINFO("Unable to set encryption policy for %s\n", TWRP_AB_LOGS_DIR);
					LOGINFO("Removing %s\n", TWRP_AB_LOGS_DIR);
					int ret = TWFunc::removeDir(TWRP_AB_LOGS_DIR, false);
					if (ret == -1) {
