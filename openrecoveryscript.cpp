...
int OpenRecoveryScript::run_script_file(void) {
  ...
  if (sideload) ret_val = 1;  // Forces booting to the home page after sideload
  return ret_val;
}
...
void OpenRecoveryScript::Run_CLI_Command(const char* command) {
  ...
  if (cmd_str == "runscript") {
    ...
    if (parts.size() == 1) {
      ...
      string pass = parts[1];
      string userid = "0";
      if (parts.size() > 2) userid = parts[2];
