...
// void res_free_surface(constGRSurface* surface) {
  // free(surface);
// }
...
static GGLSurface* convertGRSurfaceToGGLSurface(const GRSurface* grsurface) {
    GGLSurface* surface =  init_display_surface(grsurface->width, grsurface->height);
    return surface;
}

// Scale image function
int res_scale_surface(const GRSurface* source, const GRSurface* destination, float scale_w, float scale_h) {
    GGLContext *gl = NULL;
    GGLSurface* sc_mem_surface = NULL;
    destination = NULL;
    GGLSurface *surface = convertGRSurfaceToGGLSurface(source);
...
//                    w,   h,    dx,   dy,   dw,   dh,   dsdx,   dtdy, grad[0], grad[1], grad[3], grad[5]);
    ...
    destination = convertGGLSurfaceToGRSurface(sc_mem_surface).get();
