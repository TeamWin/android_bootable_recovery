...
struct StringCacheKey {
    ...
    bool operator<(const StringCacheKey &sck) const {
        return max_width < sck.max_width || (max_width == sck.max_width && std::strcmp(text, sck.text) < 0)
        || (max_width == sck.max_width && std::strcmp(text, sck.text) == 0);
    }
};
