...
int GUIAction::reflashtwrp(std::string arg __unused)
{
	...
	if (!simulate)
	{
		if (!TWFunc::Path_Exists("/twres/ramdisk-files.txt")) {
		...
		Repack_Options.Backup_First = DataManager::GetIntValue("tw_repack_backup_first") != 0;
		std::string command = "cd / && /system/bin/cpio -H newc -o < twres/ramdisk-files.txt > /tmp/currentramdisk.cpio && /system/bin/gzip /tmp/currentramdisk.cpio";
