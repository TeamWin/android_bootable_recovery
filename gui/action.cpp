...
int GUIAction::reflashtwrp(std::string arg __unused)
{
	...
	if (!simulate)
	{
		...
		gui_msg(Msg(msg::kError, "ramdisk files have been modified, unable to create ramdisk to flash, fastboot boot twrp and try this option again or use the Install Recovery Ramdisk option"));
			goto exit;
		}
		std::string command = "cd / && /system/bin/cpio -H newc -o < ramdisk-files.txt > /tmp/currentramdisk.cpio && /system/bin/gzip /tmp/currentramdisk.cpio";
