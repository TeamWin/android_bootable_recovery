...
string TWFunc::Check_For_TwrpFolder(){
	...
	if(oldFolder == "" && customTWRPFolders.empty()){
		LOGINFO("No TWRP folder found. Using TWRP as default.\n");
		goto exit;
	}else if(customTWRPFolders.empty()){
		LOGINFO("No custom TWRP folder found. Using TWRP as default.\n");
		goto exit;
	}else{
		if(customTWRPFolders.size() > 1){
			LOGINFO("More than one custom TWRP folder found. Using first one from the list.\n");
		}else{
			LOGINFO("One custom TWRP folder found.\n");
		}
