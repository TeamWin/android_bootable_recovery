...
bool TWFunc::Get_Service_Ver_From(std::string partition, std::string service, std::string &res, bool alreadyMounted) {
	LOGINFO("Looking for service %s in manifest\n", service.c_str());
	TWPartition* part = PartitionManager.Find_Partition_By_Path(partition);
	partition = partition + "/etc/vintf/manifest";
	std::string path, platform = android::base::GetProperty("ro.board.platform", ""), legacyManifest = "/vendor/etc/vintf/manifest.xml";
	...
	Exec_Cmd("find " + partition + "/ -type f -name *" + service + "*", path, false);
	if (path.empty()) {
		LOGINFO("Separate manifest doesn't exist for %s\n", service.c_str());
		// Look for main manifest*.xml
		path = partition + "_" + platform + ".xml";

		// Goto legacy manifest if platform manifest is not found.
		if (!Path_Exists(path)) {
			std::string propVal = android::base::GetProperty(TW_KEYMASTER_VERSION_PROP, "");
			if (!propVal.empty()) {
				res = propVal;
				goto finish;
			}
			...
			path = legacyManifest;
			// Rollback to Ramdisk Manifest if not Found.
			if (!Path_Exists(path)) {
				part->UnMount(true);
			}
		}
