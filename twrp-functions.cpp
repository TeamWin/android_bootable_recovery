...
bool TWFunc::Get_Service_From_Manifest(std::string basepath, std::string service, std::string &res, bool alreadyMounted) {
	std::string manifestpath, filename, platform = android::base::GetProperty("ro.board.platform", "");
	...
	if (filename.empty()) {
		LOGINFO("Keymaster_Ver::Separate manifest doesn't exist for '%s'\n", service.c_str());
		...
		if (!Path_Exists(filename)) {
			...
			LOGINFO("Keymaster_Ver::%s not found. Using default path for manifest.xml\n", filename.c_str());
			filename = manifestpath + "manifest.xml";
		}
	}
	if (Path_Exists(filename)) {
		LOGINFO("Keymaster_Ver::Looking for '%s' service in manifest\n", service.c_str());
		LOGINFO("Keymaster_Ver::Using manifest path: %s\n", filename.c_str());
				...
				if (type == "hal") {
					...
					if (type == service) {
						...
						if (versionNode != nullptr) {
							LOGINFO("Keymaster_Ver::Found version in manifest: %s\n", versionNode->value());
						} else {
							versionNode = child->first_node("fqname");
							if (versionNode == nullptr) return ret;
							LOGINFO("Keymaster_Ver::Found fqname in manifest: %s\n", versionNode->value());
						}
						...
						LOGINFO("Keymaster_Ver::Keymaster version from manifest: '%s'\n", res.c_str());
						ret = true;
					}
				}
			}
