...
static bool mount_via_fs_mgr(const char* mount_point, const char* blk_device) {
    ...
    LOG(ERROR) << "mount_via_fs_mgr::Mounted " << mount_point;
    return true;
}
...
static bool read_key(const std::string& metadata_key_dir, const KeyGeneration& gen,
    ...
    LOG(ERROR) << "metadata_key_dir/key: " << dir;
...
static bool create_crypto_blk_dev(const std::string& dm_name, const std::string& blk_device,
    ...
    *nr_sec &= ~7;
    ...
    std::stringstream ss;
    ss << *crypto_blkdev;
    LOG(ERROR) << "Created device: " << ss.str();
    return true;
}
...
bool fscrypt_mount_metadata_encrypted(const std::string& blk_device, const std::string& mount_point,
                                      bool needs_encrypt) {
    LOG(ERROR) << "fscrypt_mount_metadata_encrypted: " << mount_point << " " << needs_encrypt;
    ...
    if (needs_encrypt) {
        LOG(ERROR) << "Beginning inplace encryption, nr_sec: " << nr_sec;
        ...
        LOG(ERROR) << "Inplace encryption complete";
    }
