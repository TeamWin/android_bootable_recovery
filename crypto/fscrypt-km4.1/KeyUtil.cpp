...
bool isFsKeyringSupported(void) {
    ...
    if (!initialized) {
        ...
        if (errno == ENOTTY) {
            ...
            LOG(ERROR) << "Detected support for FS_IOC_ADD_ENCRYPTION_KEY";
            supported = true;
...
static bool installKeyLegacy(const KeyBuffer& key, const std::string& raw_ref) {
    ...
    for (char const* const* name_prefix = NAME_PREFIXES; *name_prefix != nullptr; name_prefix++) {
        ...
        LOG(ERROR) << "Added key " << key_id << " (" << ref << ") to keyring " << device_keyring
                   << " in process " << getpid();
    }
    return true;
}
...
static bool installProvisioningKey(const KeyBuffer& key, const std::string& ref,
    ...
    LOG(ERROR) << "Added fscrypt-provisioning key for " << ref << " to session keyring";
    return true;
}
...
bool installKey(const std::string& mountpoint, const EncryptionOptions& options,
    ...
    LOG(ERROR) << "Installed fscrypt key with ref " << ref << " to " << mountpoint;
...
bool reloadKeyFromSessionKeyring(const std::string& mountpoint, const EncryptionPolicy& policy) {
    ...
    LOG(ERROR) << "Installing fscrypt-provisioning key for " << ref << " back into " << mountpoint
               << " fs-keyring";
