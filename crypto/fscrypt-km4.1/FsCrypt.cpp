...
static std::vector<std::string> get_ce_key_paths(const std::string& directory_path) {
    ...
    for (;;) {
        ...
        if (entry->d_type != DT_DIR || entry->d_name[0] != 'c') {
            LOG(ERROR) << "Skipping non-key " << entry->d_name;
            continue;
        }
...
static void fixate_user_ce_key(const std::string& directory_path, const std::string& to_fix,
    ...
    if (to_fix != current_path) {
        LOG(ERROR) << "Renaming " << to_fix << " to " << current_path;
...
static bool read_and_fixate_user_ce_key(userid_t user_id,
    ...
    for (auto const ce_key_path : paths) {
        LOG(ERROR) << "Trying user CE key " << ce_key_path;
        if (retrieveKey(ce_key_path, auth, ce_key)) {
            LOG(ERROR) << "Successfully retrieved key";
...
static bool IsEmmcStorage(const std::string& blk_device) {
    ...
    LOG(ERROR) << "IsEmmcStorage(): blk_device = " << blk_device << ", real_path=" << real_path;
...
static bool read_and_install_user_ce_key(userid_t user_id,
    ...
    LOG(ERROR) << "Installed ce key for user " << user_id;
    return true;
}

static bool prepare_dir(const std::string& dir, mode_t mode, uid_t uid, gid_t gid) {
    LOG(ERROR) << "Preparing: " << dir;
...
static bool create_and_install_user_keys(userid_t user_id, bool create_ephemeral) {
    ...
    if (!install_storage_key(DATA_MNT_POINT, options, ce_key, &ce_policy)) return false;
    s_ce_policies[user_id] = ce_policy;
    LOG(ERROR) << "Created keys for user " << user_id;
    return true;
}
...
static bool load_all_de_keys() {
    ...
    for (;;) {
        ...
        if (entry->d_type != DT_DIR || !is_numeric(entry->d_name)) {
            LOG(ERROR) << "Skipping non-de-key " << entry->d_name;
            continue;
        }
        ...
        LOG(ERROR) << "Installed de key for user " << user_id;
...
bool fscrypt_initialize_systemwide_keys() {
    LOG(ERROR) << "fscrypt_initialize_systemwide_keys";
...
bool fscrypt_init_user0() {
    ...
    LOG(ERROR) << "attempting fscrypt_prepare_user_storage";
    ...
    if (!fscrypt_is_native() && !fscrypt_is_emulated()) {
        LOG(ERROR) << "unlocking data media";
...
bool fscrypt_vold_create_user_key(userid_t user_id, int serial, bool ephemeral) {
    LOG(ERROR) << "fscrypt_vold_create_user_key for " << user_id << " serial " << serial;
    if (!fscrypt_is_native()) {
        return true; 
    }
...
bool fscrypt_destroy_user_key(userid_t user_id) {
    LOG(ERROR) << "fscrypt_destroy_user_key(" << user_id << ")";
    if (!fscrypt_is_native()) {
        return true;
    }
    bool success = true;
...
static bool fscrypt_rewrap_user_key(userid_t user_id, int serial,
    ...
    if (retrieveKey(ce_key_current_path, retrieve_auth, &ce_key)) {
        ...
        LOG(ERROR) << "Successfully retrieved key with empty auth";
    } else {
...
bool fscrypt_add_user_key_auth(userid_t user_id, int serial, const std::string& token_hex,
    ...
    LOG(ERROR) << "fscrypt_add_user_key_auth " << user_id << " serial=" << serial
...
bool fscrypt_clear_user_key_auth(userid_t user_id, int serial, const std::string& token_hex,
                                 const std::string& secret_hex) {
    LOG(ERROR) << "fscrypt_clear_user_key_auth " << user_id << " serial=" << serial
               << " token_present=" << (token_hex != "!");
    if (!fscrypt_is_native()) return true;
    auto auth = authentication_from_hex(token_hex, secret_hex);
    if (!auth) return false;
...
bool fscrypt_fixate_newest_user_key_auth(userid_t user_id) {
    LOG(ERROR) << "fscrypt_fixate_newest_user_key_auth " << user_id;
    if (!fscrypt_is_native()) return true;
    if (s_ephemeral_users.count(user_id) != 0) return true;
    auto const directory_path = get_ce_key_directory_path(user_id);
    auto const paths = get_ce_key_paths(directory_path);
...
bool fscrypt_unlock_user_key(userid_t user_id, int serial, const std::string& token_hex,
                             const std::string& secret_hex) {
    LOG(ERROR) << "fscrypt_unlock_user_key " << user_id << " serial=" << serial
               << " token_present=" << (token_hex != "!");
    if (fscrypt_is_native()) {
        if (s_ce_policies.count(user_id) != 0) {
...
bool fscrypt_lock_user_key(userid_t user_id) {
    LOG(ERROR) << "fscrypt_lock_user_key " << user_id;
    if (fscrypt_is_native()) {
...
bool fscrypt_prepare_user_storage(const std::string& volume_uuid, userid_t user_id, int serial,
                                  int flags) {
    LOG(ERROR) << "fscrypt_prepare_user_storage for volume " << escape_empty(volume_uuid)
...
bool fscrypt_destroy_user_storage(const std::string& volume_uuid, userid_t user_id, int flags) {
    LOG(ERROR) << "fscrypt_destroy_user_storage for volume " << escape_empty(volume_uuid)
...
static bool destroy_volume_keys(const std::string& directory_path, const std::string& volume_uuid) {
    ...
    for (;;) {
        ...
        if (entry->d_type != DT_DIR || entry->d_name[0] == '.') {
            LOG(ERROR) << "Skipping non-user " << entry->d_name;
            continue;
        }
...
bool fscrypt_destroy_volume_keys(const std::string& volume_uuid) {
    bool res = true;
    LOG(ERROR) << "fscrypt_destroy_volume_keys for volume " << escape_empty(volume_uuid);
    auto secdiscardable_path = volume_secdiscardable_path(volume_uuid);
