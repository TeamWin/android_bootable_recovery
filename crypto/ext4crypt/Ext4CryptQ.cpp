...
/*bool fscrypt_vold_create_user_key(userid_t user_id, int serial, bool ephemeral) {
    LOG(DEBUG) << "TWRP NOT fscrypt_vold_create_user_key for " << user_id << " serial " << serial;
    return true;
    if (!fscrypt_is_native()) {
        return true;
    }
...
bool fscrypt_destroy_user_key(userid_t user_id) {
    LOG(DEBUG) << "NOT fscrypt_destroy_user_key(" << user_id << ")";
    return true;
    if (!fscrypt_is_native()) {
        return true;
    }
...
/*bool fscrypt_lock_user_key(userid_t user_id) {
    LOG(DEBUG) << "TWRP NOTfscrypt_lock_user_key " << user_id;
    return true;
    if (fscrypt_is_native()) {
...
/*bool fscrypt_destroy_user_storage(const std::string& volume_uuid, userid_t user_id, int flags) {
    LOG(DEBUG) << "TWRP NOT fscrypt_destroy_user_storage for volume " << escape_empty(volume_uuid)
...
bool fscrypt_destroy_volume_keys(const std::string& volume_uuid) {
    bool res = true;
    LOG(DEBUG) << "TWRP NOT fscrypt_destroy_volume_keys for volume " << escape_empty(volume_uuid);
    /*return res;
    auto secdiscardable_path = volume_secdiscardable_path(volume_uuid);
